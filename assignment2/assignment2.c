...
int main( int argc, char *argv[] )
{
  ...
  int mode, lastn;
  char *modeInput;
  const char *encKeyword = "encryption";
  const char *decKeyword = "decryption";
  size_t keylen;
  ...
  */
  ...
  char *inputSearchPointer = strcasestr(encKeyword, modeInput);
  if (inputSearchPointer != NULL)
  {
    mode=MODE_ENCRYPT;
  }
  else
  {
    char *inputSearchPointer = strcasestr(decKeyword, modeInput);
    if (inputSearchPointer != NULL)
    {
      mode=MODE_DECRYPT;
    }
